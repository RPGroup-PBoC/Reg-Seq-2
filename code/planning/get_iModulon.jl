using CSV, DataFrames, wgregseq, CairoMakie, Pandas
...
df_samples = CSV.read("/$home_dir/data/iModulon/sample_table.csv", DataFrame)
df_samples[!, ["Carbon Source (g/L)"]] |> unique |> println
display(df_samples[!, ["Strain Description"]] |> unique)
...
## iModulon metadata
df_im = CSV.read("/$home_dir/data/iModulon/iM_table.csv", DataFrame)
names(df_im)
...
## Normalized reads 
df = CSV.read("/$home_dir/data/iModulon/A.csv", DataFrame)
