...
RAW_DIR = INPUT_DIR + '220809_M06105_0039_000000000-KHNL8/Data/Intensities/BaseCalls/'
...
os.rename(FORWARD_SEQ, TMP_DIR + 'sequences.fastq.gz')
...
# Run subprocess activating qiime2 conda environmentß
print(f'generating qiime2 artifact on conda {CONDA_ENV} environment.')
subprocess.run(f"bash -c 'source activate {CONDA_ENV} && {bash_artifact}'", shell=True)

# Rename files back to original name
print('renaming fastq files back to original name...')
os.rename(TMP_DIR + 'sequences.fastq.gz', FORWARD_SEQ)
...
# cannot be applied
bash_demux = f'''qiime demux emp-single \
...
if not os.path.exists(demux_viz):
    ...
    subprocess.run(f"bash -c 'source activate {CONDA_ENV} && {bash_viz}'", shell=True)
